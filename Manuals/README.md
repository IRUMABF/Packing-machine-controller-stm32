...
![Загальний вигляд](general_view.jpg)
...
|         | GND     | Земля датчика 3 | |
| Кнопки  | Stop    | Нормально розімкнена (NO) | Підключається до входу STM32 (PA1) з підтяжкою до + |
|         | Start   | Нормально розімкнена (NO) | Підключається до входу STM32 (PA0) з підтяжкою до + |
...
|---------|--------|-------------|----------------------|
| LED1    | | Червона/жовта лампа "немає пакетів" | PA12 (через резистор 220 Ом до GND) |
| LED2    | | Зелена лампа стану (LED1) | PA13 (через резистор) |
...
- Живлення клапанів – 12V DC (зовнішнє).  
